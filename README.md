...
BulkCart は Vercel の無料枠で動作するように設計されています。

#### クイックスタート

1. [Vercel](https://vercel.com/) でアカウント作成（GitHub連携推奨）
2. GitHubリポジトリを接続
3. Environment Variablesを設定（[環境変数チェックリスト](docs/environment-variables-checklist.md) 参照）
4. **Deploy** をクリック
...
- `NEXT_PUBLIC_APP_URL` - デプロイ先URL（デプロイ後に設定）
- `NODE_ENV` - `production`

詳細は以下のドキュメントを参照：

- **📘 [デプロイ手順書](docs/deployment.md)** - 15分で完了する詳細ガイド
- **📋 [環境変数チェックリスト](docs/environment-variables-checklist.md)** - 設定漏れ防止
- **🔧 [Upstash Redis セットアップ](docs/upstash-setup.md)** - キャッシュ設定（5分）

### 本番環境チェックリスト

- [ ] Supabase Production プロジェクト作成済み（[supabase-setup.md](docs/supabase-setup.md)）
- [ ] データベースマイグレーション実行済み（`npx supabase db push`）
- [ ] Upstash Redis データベース作成済み（[upstash-setup.md](docs/upstash-setup.md)）
- [ ] Vercel 環境変数が設定済み（8個すべて）
- [ ] 初回デプロイ成功、動作確認済み
- [ ] カスタムドメイン設定済み（オプション）
...
| サービス | プラン | 制限 | 月額コスト |
|---------|--------|------|-----------|
| Vercel | Hobby | デプロイ無制限、100GB帯域 | **$0** |
| Supabase | Free | 500MB DB、50k MAU | **$0** |
| Upstash | Free | 10GB/月、10k コマンド/日 | **$0** |
| **合計** | - | **~100ユーザー対応** | **$0/月** |

> **💡 ヒント**: Phase 10（Stripe統合）後は、取引手数料のみ発生（3.6%）
...
- **📘 [Supabaseセットアップ](docs/supabase-setup.md)** - データベース作成と初期設定（10分）
- **📘 [Upstash Redis セットアップ](docs/upstash-setup.md)** - キャッシュ設定（5分）
- **🚀 [デプロイ手順書](docs/deployment.md)** - Vercel へのデプロイガイド（15分）
- **📋 [環境変数チェックリスト](docs/environment-variables-checklist.md)** - デプロイ前確認
- パフォーマンス最適化（Phase 8.2で作成予定）
- トラブルシューティング（Phase 8.4で作成予定）
