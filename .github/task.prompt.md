...
### 5.1 🤖 レシピデータベース構築
- [ ] recipes, ingredients, recipe_ingredients テーブル作成 SQL
- [ ] 初期レシピデータ（docs/seed-data/recipes.csv の50件を使用）
- [ ] CSV → DB 投入スクリプト作成（seed script）
- [ ] DB インデックス最適化 SQL
- [ ] レシピ検索関数実装（`lib/recipe/search.ts`）
  - タグフィルタ、難易度フィルタ、調理時間フィルタ
  - アレルギー除外ロジック
- [ ] `supabase/migrations/` と `supabase/seed.sql` に保存
...
### 5.4 🤖 作り置き段取り生成
- [ ] 献立→調理タスクの逆算ロジック（`lib/planner/prep.ts`）
- [ ] 週 1 回バッチ想定のタイムライン生成
  - 例: 日曜 60分（00:00 米炊く、00:05 鶏むね下処理...）
- [ ] API エンドポイント実装 (`app/api/plan/[id]/prep/route.ts`)
- [ ] タイムライン表示形式の型定義
...
### 6.1 🤖 買い物リスト画面実装
- [ ] カテゴリ別テーブル表示（肉/魚/卵乳/野菜/主食/調味料）
- [ ] チェックボックス（ローカル State または localStorage）
- [ ] 「コピー」ボタン（クリップボード API）
- [ ] 「LINE に送る」ボタン（共有リンク生成）
- [ ] 「印刷用 PDF」ボタン（ブラウザ印刷 or PDF 生成）
- [ ] `app/(app)/plan/[week]/grocery/page.tsx` として実装
